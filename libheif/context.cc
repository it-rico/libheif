...
Result<std::shared_ptr<ImageItem>> HeifContext::add_image(std::shared_ptr<HeifContext>& in_ctx, std::shared_ptr<ImageItem>& in_image)
{
  ...
  if (in_image->get_infe_type() == fourcc("grid")) {
    auto addResult = add_image_from_grid(in_ctx, in_image);
    if (addResult.error) {
      return addResult.error;
    }
    out_image = addResult.value;
    // Set Brands
  ...
  m_heif_file->set_brand(in_image->get_compression_format(), out_image->is_miaf_compatible());
  return out_image;
}
